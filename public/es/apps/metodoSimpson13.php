...
<html lang="es">
  <head>
    ...
    <title>Método de Simpson 1/3 | Omega Academy</title>
  ...
  <body>
    ...
    <div class="container">
      ...
      <form method="POST" action="metodoSimpson13.php" class="form-horizontal" role="form">          
        <legend><h2 class="text-center">Método de Simpson 1/3</h2></legend>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-3">
            <input name="particiones" type="number" class="form-control" id="particiones" min="2" step="2" <?php 
...
<?php 
  if (isset($_POST["funcion"])) {
    ...
    function simpson13($a, $b, $h, $funcion, $eval) {
      $x0 = $a;
      $f0 = $eval->expression($funcion, $x0);

      $m = $x0 + $h;
      $fm = $eval->expression($funcion, $m);

      $x2 = $m + $h;
      $f2 = $eval->expression($funcion, $x2);

      if ($x2 >= $b) {        
        return ($h / 3) * ($f0 + (4*$fm) + $f2);
      }
      else return simpson13($x2, $b, $h, $funcion, $eval);
...
?>
      ...
      <div style="text-align: center;">
        <a id="boton" href="https://www.youtube.com/watch?v=Q4Pk4e73jF0" target="_blank" type="button" class="btn btn-lg" style="background: gray; color: white">Vídeo</a>
        <a id="boton" href="../documentos/unidad11.pdf" target="_blank" type="button" class="btn btn-lg" style="background: #D40B3A; color: white">Documento</a>        
      </div>
