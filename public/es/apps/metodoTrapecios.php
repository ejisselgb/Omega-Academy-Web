  ...
  <body>
    ...
    <div class="container">
      ...
      <form method="POST" action="metodoTrapecios.php" class="form-horizontal" role="form">          
        ...
        <div class="form-group">
          <label for="trapecios" class="col-sm-5 control-label">No. de trapecios</label>
          <div class="col-sm-3">
            <input name="trapecios" type="number" class="form-control" id="trapecios" min="1" max="20" <?php 
      ...
      <?php 
        if (isset($_POST['funcion'])) {
          ...
          function integrate($a, $b, $trapecios, $funcion, $eval) {
            $h = ($b - $a) / $trapecios;
            $suma = ($eval->expression($funcion, $a) + $eval->expression($funcion, $b)) / 2;
            for ($i=1; $i < $trapecios; $i++) { 
              $suma += $eval->expression($funcion, ($a + ($i * $h) ));
            }
            return $suma * $h;
          }
        ...
        //echo $eval->expression($funcion, (float) $b);
      ?>
